...
describe('Harness AI Builder',()=>{
  it('creates a session, generates, validates and previews through the shared renderer',async()=>{const calls:string[]=[];configureHarnessTransport(async(url,init)=>{calls.push(url);
    ...
    if(url==='/harness/ai/sessions')return structuredClone(session);
  ...
  it('shows stable provider error codes instead of hiding generation failure',async()=>{configureHarnessTransport(async(url)=>{if(url.endsWith('/status'))return {enabled:true};if(url==='/harness/apps'||url==='/harness/capabilities')return [];if(url==='/harness/ai/sessions')return structuredClone(session);if(url.endsWith('/messages'))throw {code:'AI_RATE_LIMITED',message:'Try later'};return {}});
